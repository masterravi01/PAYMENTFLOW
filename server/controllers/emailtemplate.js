...
const sendPasswordResetLink = async (email, resetUrl) => {
    try {
        ...
        const mailOptions = {
            ...
            html: `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Password Reset</title><style>/* Reset CSS */body,html {margin: 0;padding: 0;font-family: Arial, sans-serif;}/* Main container */.container {max-width: 600px;margin: 0 auto;padding: 20px;}/* Header */.header {text-align: center;margin-bottom: 20px;}/* Body */.body {margin-bottom: 20px;}/* Footer */.footer {text-align: center;font-size: 12px;}</style></head><body><div class="container"><div class="header"><h1>Password Reset</h1></div><div class="body"><p>Hello,</p><p>We received a request to reset your password. Please click the link below to reset your password:</p><p>${resetUrl}</p><p>If you did not request a password reset, you can ignore this email.</p><p>Sincerely,</p><p>Rsp Technical Team</p></div><div class="footer"><p>We never send misleading emails to customers.</p></div></div></body></html>`,
        };
